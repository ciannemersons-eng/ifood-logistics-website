...
export function MobileMenu({ navItems, cta }: { navItems: NavItem[]; cta: Cta }) {
  ...
  return (
    <div className="md:hidden">
      ...
      {isOpen
        ? createPortal(
            // Rendered via portal directly into <body>, not as a descendant of
            // <header>. The header has backdrop-blur-sm (a backdrop-filter)
            // when unscrolled, and per the CSS spec a backdrop-filter/filter/
            // transform on an ancestor becomes the containing block for any
            // descendant position:fixed element — so this panel's "fixed
            // inset-0" was resolving relative to the ~80px-tall header box
            // instead of the viewport, leaving most of it unpainted and the
            // page content showing through. Portaling out of the header
